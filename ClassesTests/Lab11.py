class BinarySearchTreeMap:
    ...
    def SInsert(self, root, key, value):

        if root.left and root.right is None:
            newI = BinarySearchTreeMap.Item(key, value)
            newNode = BinarySearchTreeMap.Node(newI)
            ...
            if key < root.item.key:
                self.Sinsert(root.left, key, value)
            else:
                self.Sinsert(root.right, key, value)

    def insert(self, key, value):
        newI = BinarySearchTreeMap.Item(key, value)
        newNode = BinarySearchTreeMap.Node(newI)
        while root.left or root.right is not None:
